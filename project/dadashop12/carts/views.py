...
class CartsView(View):
    ...
    def put(self, request, username):
        # 选中 {"sku_id":"3","state":"select"}
        # 未选中 {"sku_id":"1","state":"unselect"}
        # 全选 {"state":"selectall"}
        # 全不选　{"state":"unselectall"}

        user = request.myuser
        data = json.loads(request.body.decode())

        carts_state = data["state"]
        key = self.get_cache_key(user.id)
        dict_all_data = self.get_carts_all_data(user.id)

        if carts_state == "selectall" or carts_state == "unselectall":
            # 全选 或者　全不选
            for data in dict_all_data:
                dict_all_data[data][1] = 1 if carts_state == "selectall" else 0

        elif carts_state == "select" or carts_state == "unselect":
            # 选中　或者　未选中
            carts_sku_id = int(data["sku_id"])
            dict_all_data[carts_sku_id][1] = 1 if carts_state == "select" else 0

        # print("-------")
        # print(dict_all_data)
        CARTS_CACHE.set(key, dict_all_data)
